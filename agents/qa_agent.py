...
def run(state: ForgeState) -> ForgeState:
    ...
    # Generate conftest.py deterministically before writing any test files.
    # This ensures fixtures (client, db, session, auth_token) exist for every test.
    conftest_path = _find_tests_dir(all_project_files) + "/conftest.py"
    test_files[conftest_path] = _build_conftest(architecture, spec, all_project_files, tier)
    log_action("QAAgent", f"Generated {conftest_path} (deterministic)")
...
def _find_tests_dir(all_project_files: set) -> str:
    """Return the tests directory used by this project, defaulting to 'tests'."""
...
def _build_conftest(architecture: dict, spec: dict, all_project_files: set, tier: int) -> str:
    """
    Build conftest.py content deterministically from the project architecture.
    No LLM call — content is fully derivable from known stack patterns.

    Provides:
      client      — FastAPI TestClient (always)
      db/session  — SQLAlchemy in-memory Session (if relational DB detected)
      mock_collection — fake MongoDB collection (if Mongo detected)
      auth_token / admin_token — JWT helpers (Tier 4+)
    """
    stack_str = str(spec.get("inferred_stack", {})).lower()
    arch_str  = str(architecture.get("key_decisions", [])).lower()
    combined  = stack_str + " " + arch_str

    use_mongo = any(x in combined for x in ("mongo", "motor"))
    use_sql   = not use_mongo  # SQLite or Postgres both use SQLAlchemy session fixture

    # Locate main app module
    main_module = "app.main"
    for fp in sorted(all_project_files):
        if fp.endswith("main.py"):
            main_module = fp.replace("/", ".").removesuffix(".py")
            break

    # Locate database module
    db_module = "app.database"
    for fp in sorted(all_project_files):
        if "database" in fp and fp.endswith(".py"):
            db_module = fp.replace("/", ".").removesuffix(".py")
            break
    ...
    # ── Header ───────────────────────────────────────────────────────────────
    parts.append('"""')
    parts.append("conftest.py — shared pytest fixtures for the generated project.")
    parts.append("Auto-generated by Forge QAAgent. All imports are guarded so")
    parts.append("tests can be collected even before implementation files exist.")
    parts.append('"""')
    ...
    # ── client fixture ────────────────────────────────────────────────────────
    parts.append("# ── HTTP test client ─────────────────────────────────────────────────")
    parts.append("try:")
    parts.append(f"    from {main_module} import app")
    parts.append("    from starlette.testclient import TestClient")
    parts.append("    _HAS_APP = True")
    parts.append("except Exception:")
    parts.append("    _HAS_APP = False")
    parts.append("")
    parts.append("")
    parts.append('@pytest.fixture(scope="module")')
    parts.append("def client():")
    parts.append('    """TestClient for the FastAPI app."""')
    parts.append("    if not _HAS_APP:")
    parts.append('        pytest.skip("app not yet implemented")')
    parts.append("    with TestClient(app) as c:")
    parts.append("        yield c")
    parts.append("")

    # ── db / session fixtures ─────────────────────────────────────────────────
    if use_sql:
        parts.append("# ── SQLAlchemy in-memory DB ──────────────────────────────────────────")
        parts.append("try:")
        parts.append(f"    from {db_module} import Base")
        parts.append("    from sqlalchemy import create_engine")
        parts.append("    from sqlalchemy.orm import sessionmaker")
        parts.append("    _HAS_DB = True")
        parts.append("except Exception:")
        parts.append("    _HAS_DB = False")
        parts.append("")
        parts.append("")
        parts.append("@pytest.fixture()")
        parts.append("def db():")
        parts.append('    """Isolated in-memory SQLite session, reset per test."""')
        parts.append("    if not _HAS_DB:")
        parts.append('        pytest.skip("database not yet implemented")')
        parts.append('    engine = create_engine(')
        parts.append('        "sqlite:///:memory:",')
        parts.append('        connect_args={"check_same_thread": False},')
        parts.append('    )')
        parts.append("    Base.metadata.create_all(engine)")
        parts.append("    Session = sessionmaker(bind=engine)")
        parts.append("    session = Session()")
        parts.append("    try:")
        parts.append("        yield session")
        parts.append("    finally:")
        parts.append("        session.close()")
        parts.append("        Base.metadata.drop_all(engine)")
        parts.append("")
        parts.append("")
        parts.append("@pytest.fixture()")
        parts.append("def session(db):")
        parts.append('    """Alias for the db fixture."""')
        parts.append("    return db")
        parts.append("")

    # ── MongoDB mock fixture ──────────────────────────────────────────────────
    if use_mongo:
        parts.append("# ── MongoDB mock collection ──────────────────────────────────────────")
        parts.append("@pytest.fixture()")
        parts.append("def mock_collection():")
        parts.append('    """Minimal async-compatible fake MongoDB collection."""')
        parts.append("")
        parts.append("    class _FakeResult:")
        parts.append('        inserted_id = "fake_id"')
        parts.append("")
        parts.append("    class _FakeCollection:")
        parts.append("        async def find_one(self, query=None):")
        parts.append("            return None")
        parts.append("")
        parts.append("        async def insert_one(self, doc):")
        parts.append("            return _FakeResult()")
        parts.append("")
        parts.append("        async def find(self, query=None):")
        parts.append("            return iter([])")
        parts.append("")
        parts.append("        async def delete_one(self, query):")
        parts.append("            return type('r', (), {'deleted_count': 1})()")
        parts.append("")
        parts.append("        async def update_one(self, query, update, upsert=False):")
        parts.append("            return type('r', (), {'modified_count': 1})()")
        parts.append("")
        parts.append("    return _FakeCollection()")
        parts.append("")

    # ── JWT auth helpers (Tier 4+) ────────────────────────────────────────────
    if tier >= 4:
        parts.append("# ── Auth helpers ─────────────────────────────────────────────────────")
        parts.append("try:")
        parts.append("    from jose import jwt as _jwt")
        parts.append("    import os as _os")
        parts.append("    _SECRET = _os.getenv('SECRET_KEY', 'test-secret-key')")
        parts.append("    _ALGORITHM = 'HS256'")
        parts.append("    _HAS_JWT = True")
        parts.append("except Exception:")
        parts.append("    _HAS_JWT = False")
        parts.append("")
        parts.append("")
        parts.append("@pytest.fixture()")
        parts.append("def auth_token():")
        parts.append('    """Valid JWT bearer token for role=user."""')
        parts.append("    if not _HAS_JWT:")
        parts.append('        pytest.skip("jose not installed")')
        parts.append("    from datetime import datetime, timedelta")
        parts.append("    payload = {")
        parts.append('        "sub": "testuser",')
        parts.append('        "role": "user",')
        parts.append('        "exp": datetime.utcnow() + timedelta(hours=1),')
        parts.append("    }")
        parts.append("    return _jwt.encode(payload, _SECRET, algorithm=_ALGORITHM)")
        parts.append("")
        parts.append("")
        parts.append("@pytest.fixture()")
        parts.append("def admin_token():")
        parts.append('    """Valid JWT bearer token for role=admin."""')
        parts.append("    if not _HAS_JWT:")
        parts.append('        pytest.skip("jose not installed")')
        parts.append("    from datetime import datetime, timedelta")
        parts.append("    payload = {")
        parts.append('        "sub": "adminuser",')
