...
**⚙️ Priority Weights (optional)** — expand to adjust 5 sliders that bias agent decisions:
...
- Task ID and title
- Status: ⏳ Pending → ⚙️ In Progress → ✅ Done / ❌ Failed
...
- Issue description
- Whether `SecurityFixAgent` automatically applied a fix (✅ Fixed / ⚠️ Open)
...
- **✏️ AI Edit** — describe a change in plain English (e.g. `"Add a /students/summary endpoint that returns grade distribution"`) and `CodeGenAgent` applies it to the selected file automatically
...
- **File Tree** — all files with line counts and role icons (🏠 main, 🗄️ database, 🔐 auth, 🧪 tests...)
